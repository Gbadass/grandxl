...
// The plugin extends L.gridLayer with a `googleMutant` factory at import time.
// No exports to consume — side-effect import only.
import 'leaflet.gridlayer.googlemutant'

const KEY = import.meta.env.VITE_GOOGLE_MAPS_JS_KEY as string | undefined
...
function loadGoogleMapsSdk(key: string): Promise<void> {
  ...
  loadingPromise = new Promise<void>((resolve, reject) => {
    ...
    // Reset so a subsequent mount can retry after the network recovers.
    loadingPromise = null
    throw err
  })
...
interface Props {
  // 'roadmap' matches Bolt/Uber; 'satellite' + 'hybrid' available for the
  // basemap toggle. 'terrain' rarely useful for delivery UX.
