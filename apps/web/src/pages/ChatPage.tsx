...
function TypingIndicator({ name }: { name: string }) {
  return (
    ...
    >
      ...
      <span className="text-xs text-gray-400 pb-1">{name} is typing…</span>
    </motion.div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col gap-0.5 max-w-[78%] ${isOwn ? 'items-end' : 'items-start'}`}>
        ...
        >
          {msg.deletedAt ? (
            <span className="italic opacity-50 text-sm">This message was removed</span>
          ) : (
            msg.text
          )}
        </div>
...
export default function ChatPage() {
  ...
  const [peerName, setPeerName] = useState('Rider')
  ...
  useEffect(() => {
    ...
    setPeerName(isRider ? 'Customer' : 'Rider')
  }, [order, user])
        ...
        {!historyLoading && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center flex-1 px-8 text-center gap-4 mt-20">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Send a quick note to your {peerName.toLowerCase()}</p>
            </div>
      ...
      <div className="px-3 py-3 bg-white border-t border-gray-100 flex items-end gap-2 safe-area-bottom">
        ...
        <div className="flex-1 relative">
          <textarea
            ...
            placeholder={`Message ${peerName.toLowerCase()}…`}
            maxLength={500}
            rows={1}
