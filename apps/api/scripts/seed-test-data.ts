/**
 * Seeds realistic test data: 3 restaurants, menu items, 2 riders, 2 customers, 20 orders.
 *
 * Run:        npx tsx scripts/seed-test-data.ts
 * Reset only: npx tsx scripts/seed-test-data.ts --reset
 * Reset+seed: npx tsx scripts/seed-test-data.ts --reset --seed
 */
...
const SEED_EMAILS = [
  'chidi@testrestaurant.com',
  'amaka@testrestaurant.com',
  'tunde@testrestaurant.com',
  'emeka.rider@test.com',
  'bola.rider@test.com',
  'ngozi@test.com',
  'seun@test.com',
]

async function reset(db: mongoose.mongo.Db) {
  console.log('🗑️  Removing seeded data...')
  const userDocs = await db.collection('users').find({ email: { $in: SEED_EMAILS } }).toArray()
  const userIds = userDocs.map((u) => u._id)
  const restaurantDocs = await db.collection('restaurants').find({ ownerId: { $in: userIds } }).toArray()
  const restaurantIds = restaurantDocs.map((r) => r._id)
  const riderDocs = await db.collection('riders').find({ userId: { $in: userIds } }).toArray()
  const riderIds = riderDocs.map((r) => r._id)

  await Promise.all([
    db.collection('orders').deleteMany({ $or: [{ customerId: { $in: userIds } }, { riderId: { $in: riderIds } }, { restaurantId: { $in: restaurantIds } }] }),
    db.collection('menu_items').deleteMany({ restaurantId: { $in: restaurantIds } }),
    db.collection('menu_categories').deleteMany({ restaurantId: { $in: restaurantIds } }),
    db.collection('riders').deleteMany({ userId: { $in: userIds } }),
    db.collection('restaurants').deleteMany({ _id: { $in: restaurantIds } }),
    db.collection('users').deleteMany({ email: { $in: SEED_EMAILS } }),
  ])
  console.log('✅ Seeded data removed')
}

async function seed(db: mongoose.mongo.Db) {
  const hash = (pw: string) => bcrypt.hash(pw, 10)
  const id = () => new Types.ObjectId()
  const daysAgo = (n: number) => new Date(Date.now() - n * 86_400_000)

  // ── 1. Restaurant owners ─────────────────────────────────────────────────────
  const owner1Id = id(); const owner2Id = id(); const owner3Id = id()

  const owners = [
    { _id: owner1Id, firstName: 'Chidi', lastName: 'Okonkwo', email: 'chidi@testrestaurant.com', phone: '+2348011111111', roles: ['restaurant_owner'], passwordHash: await hash('Test@1234'), isVerified: true, isActive: true, avatar: null, expoPushToken: null, addresses: [], defaultAddressId: null, country: 'NG', currency: 'NGN', locale: 'en-NG', consentGiven: true, consentDate: daysAgo(30), lastLoginAt: daysAgo(1), deletedAt: null, createdAt: daysAgo(30), updatedAt: daysAgo(1) },
    { _id: owner2Id, firstName: 'Amaka', lastName: 'Eze',     email: 'amaka@testrestaurant.com', phone: '+2348022222222', roles: ['restaurant_owner'], passwordHash: await hash('Test@1234'), isVerified: true, isActive: true, avatar: null, expoPushToken: null, addresses: [], defaultAddressId: null, country: 'NG', currency: 'NGN', locale: 'en-NG', consentGiven: true, consentDate: daysAgo(20), lastLoginAt: daysAgo(2), deletedAt: null, createdAt: daysAgo(20), updatedAt: daysAgo(2) },
    { _id: owner3Id, firstName: 'Tunde', lastName: 'Adeyemi', email: 'tunde@testrestaurant.com', phone: '+2348033333333', roles: ['restaurant_owner'], passwordHash: await hash('Test@1234'), isVerified: true, isActive: true, avatar: null, expoPushToken: null, addresses: [], defaultAddressId: null, country: 'NG', currency: 'NGN', locale: 'en-NG', consentGiven: true, consentDate: daysAgo(10), lastLoginAt: daysAgo(3), deletedAt: null, createdAt: daysAgo(10), updatedAt: daysAgo(3) },
  ]
  await db.collection('users').insertMany(owners)
  console.log('✅ Restaurant owners created')

  // ── 2. Restaurants ───────────────────────────────────────────────────────────
  const r1Id = id(); const r2Id = id(); const r3Id = id()
  const openingHours = {
  ...
  const restaurants = [
    { _id: r1Id, ownerId: owner1Id, name: 'Mama Chidi Kitchen', slug: 'mama-chidi-kitchen', description: 'Authentic Nigerian home cooking.', logo: null, coverImage: null, phone: '+2348011111111', email: 'chidi@testrestaurant.com', cuisine: ['Nigerian', 'African'], address: { street: '14 Bourdillon Road', city: 'Ikoyi', state: 'Lagos', country: 'NG', coordinates: { type: 'Point', coordinates: [3.4378, 6.4487] } }, openingHours, deliveryRadius: 5, minOrderAmount: 200000, deliveryFeeType: 'fixed', deliveryFeeFixed: 80000, estimatedDeliveryTime: 35, isActive: true, isOpen: true, isApproved: true, approvalStatus: 'approved', approvalNote: null, approvedAt: daysAgo(25), approvedBy: null, rating: 4.6, ratingCount: 38, country: 'NG', currency: 'NGN', bankDetails: { bankName: 'GTBank', accountNumber: '0123456789', accountName: 'Chidi Okonkwo' }, createdAt: daysAgo(28), updatedAt: daysAgo(1) },
    { _id: r2Id, ownerId: owner2Id, name: "Amaka's Grill House", slug: 'amakas-grill-house',  description: 'Grilled chicken, suya, and barbecue.', logo: null, coverImage: null, phone: '+2348022222222', email: 'amaka@testrestaurant.com', cuisine: ['Grill', 'Nigerian', 'BBQ'], address: { street: '7 Allen Avenue', city: 'Ikeja', state: 'Lagos', country: 'NG', coordinates: { type: 'Point', coordinates: [3.3522, 6.6018] } }, openingHours, deliveryRadius: 6, minOrderAmount: 150000, deliveryFeeType: 'fixed', deliveryFeeFixed: 60000, estimatedDeliveryTime: 25, isActive: true, isOpen: true, isApproved: true, approvalStatus: 'approved', approvalNote: null, approvedAt: daysAgo(18), approvedBy: null, rating: 4.8, ratingCount: 72, country: 'NG', currency: 'NGN', bankDetails: { bankName: 'Access Bank', accountNumber: '0987654321', accountName: 'Amaka Eze' }, createdAt: daysAgo(20), updatedAt: daysAgo(2) },
    { _id: r3Id, ownerId: owner3Id, name: "Tunde's Pizza & Pasta", slug: 'tundes-pizza-pasta', description: 'Lagos-inspired pizza and pasta.', logo: null, coverImage: null, phone: '+2348033333333', email: 'tunde@testrestaurant.com', cuisine: ['Pizza', 'Italian', 'Fast Food'], address: { street: '25 Admiralty Way', city: 'Lekki', state: 'Lagos', country: 'NG', coordinates: { type: 'Point', coordinates: [3.5852, 6.4355] } }, openingHours, deliveryRadius: 7, minOrderAmount: 250000, deliveryFeeType: 'fixed', deliveryFeeFixed: 100000, estimatedDeliveryTime: 40, isActive: true, isOpen: false, isApproved: false, approvalStatus: 'pending_review', approvalNote: null, approvedAt: null, approvedBy: null, rating: 0, ratingCount: 0, country: 'NG', currency: 'NGN', bankDetails: { bankName: null, accountNumber: null, accountName: null }, createdAt: daysAgo(8), updatedAt: daysAgo(8) },
  ]
  ...
  await db.collection('menu_categories').insertMany([
    ...
    { _id: cat2, restaurantId: r1Id, name: 'Rice Dishes',   description: 'Jollof, fried rice...',   sortOrder: 1, isActive: true, createdAt: daysAgo(28), updatedAt: daysAgo(28) },
  ...
  await db.collection('users').insertMany([
    { _id: riderUser1Id, firstName: 'Emeka', lastName: 'Nwosu',   email: 'emeka.rider@test.com', phone: '+2348044444444', roles: ['rider'], passwordHash: await hash('Test@1234'), isVerified: true,  isActive: true, avatar: null, expoPushToken: null, addresses: [], defaultAddressId: null, country: 'NG', currency: 'NGN', locale: 'en-NG', consentGiven: true, consentDate: daysAgo(15), lastLoginAt: daysAgo(1), deletedAt: null, createdAt: daysAgo(15), updatedAt: daysAgo(1) },
    { _id: riderUser2Id, firstName: 'Bola',  lastName: 'Adesanya', email: 'bola.rider@test.com',  phone: '+2348055555555', roles: ['rider'], passwordHash: await hash('Test@1234'), isVerified: false, isActive: true, avatar: null, expoPushToken: null, addresses: [], defaultAddressId: null, country: 'NG', currency: 'NGN', locale: 'en-NG', consentGiven: true, consentDate: daysAgo(10), lastLoginAt: daysAgo(2), deletedAt: null, createdAt: daysAgo(10), updatedAt: daysAgo(2) },
  ])
  await db.collection('riders').insertMany([
  ...
  const makeOrder = (n: number, customerId: Types.ObjectId, restaurantId: Types.ObjectId, riderId: Types.ObjectId | null, itemName: string, itemId: Types.ObjectId, itemPrice: number, qty: number, status: string, paymentStatus: string, daysBack: number) => {
    ...
    return {
      ...
      customerId, restaurantId, riderId, status,
      items: [{ menuItemId: itemId, name: itemName, image: null, basePrice: itemPrice, quantity: qty, selectedVariants: [], selectedAddOns: [], itemTotal: itemPrice * qty, note: null }],
      ...
      timeoutJobId: null, country: 'NG', currency: 'NGN',
  ...
  console.log('   Restaurants : 3 (2 approved, 1 pending_review)')
  console.log('   Riders      : 2 (1 verified/online, 1 pending)')
  console.log('   Customers   : 2')
  console.log('   Orders      : 20 (16 delivered, 2 active, 1 cancelled, 1 pending)')
  console.log('\n   Logins (all password: Test@1234):')
  console.log('   chidi@testrestaurant.com  — restaurant owner — Mama Chidi Kitchen')
  console.log('   amaka@testrestaurant.com  — restaurant owner — Amaka\'s Grill House')
  console.log('   tunde@testrestaurant.com  — restaurant owner — Tunde\'s Pizza (pending approval)')
  console.log('   ngozi@test.com            — customer')
  console.log('   seun@test.com             — customer')
  console.log('   emeka.rider@test.com      — rider')
}

async function main() {
  const args = process.argv.slice(2)
  const doReset = args.includes('--reset')
  const doSeed  = args.includes('--seed') || !doReset
